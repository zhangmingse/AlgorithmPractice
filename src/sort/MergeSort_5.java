...
public class MergeSort_5 {
	public static void main(String [] args){
		...
		System.out.println("total time : " + (end-start) +" mill seconds");
	}
	...
	private static void merge(int start_index,
		...
		int left_ponit = start_index;
		int right_point = middle+1;
		for(int i = start_index;i<=end_index;i++){
			
			if(left_ponit>middle){
				raw[i] = temp_arr[right_point++];
				continue;
			}
			
			if(right_point > end_index){
				raw[i] = temp_arr[left_ponit];
				continue;
			}
			
			if(temp_arr[left_ponit] < temp_arr[right_point]){
				raw[i] = temp_arr[left_ponit++];
			}else{
				raw[i] = temp_arr[right_point++];
			}
